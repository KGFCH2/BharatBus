...
export default function BookTicket() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Auto-generate PDF and save in background
    setTimeout(async () => {
      try {
        ...
        try {
          ...
          saveTicketToLocal(id, null, imgData);
          console.warn('Auto-save PDF to localStorage failed', err);
        }
      } catch (err) {
                ...
                <button
                  onClick={async () => {
                    try {
                      ...
                      if (!el) return;
                      const { pdf, imgData } = await captureElement(el as HTMLElement);
                      (pdf as PdfLike).save?.(`${ticketId}.pdf`);
                      try {
                        const dataUri = (pdf as PdfLike).output('datauristring');
                        saveTicketToLocal(ticketId, dataUri, imgData);
                        alert('PDF saved to disk and stored locally in your browser.');
                      } catch (err) {
                        saveTicketToLocal(ticketId, null, imgData);
                        console.warn('Failed to store PDF in localStorage', err);
                      }
                    } catch (err) {
          ...
          <GlassCard>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <input id="phone" type="tel" name="phone" value={formData.phone} onChange={handleChange} required placeholder="+91 XXXXX XXXXX" className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/20 text-white placeholder-white/40 focus:outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-400/20 transition-all light:text-black light:placeholder-slate-600 light:bg-white light:border-gray-200" />
              </div>
